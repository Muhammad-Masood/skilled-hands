...
import { addDoc, Firestore, doc, documentId, setDoc, getDocs, collection, updateDoc, getDoc } from 'firebase/firestore';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const RevCollectionReference = collection(db, "crafters", "QXbOxuE7VkSber4lD8iw", "reviews");
    ...
    return NextResponse.json({ message: "Error storing Crafter Review" });
  }
}
...
export async function GET(request: NextRequest, response: NextResponse) {
  try {
    ...
    const ReviewsDetailsDocRef = doc(db, "crafters", "QXbOxuE7VkSber4lD8iw", "reviews", id);
